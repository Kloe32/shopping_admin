...
import { fetchUnits,fetchRates,addNewUnit,deleteUnit,updateUnit } from "../services/unit.services";
...
const Unit = () => {
  ...
  const handleUpdateUnit = async (id,payload)=>{
    try {
      ...
      if(response.success){
        toast.info(response.message)
        setUnits(units.map(u => (u._id === currentUnit._id ? { ...u, name: unitName.trim().toUpperCase() } : u)));
      }
    } catch (error) {
  ...
  const handleSubmit = (e) => {
    ...
    const isDuplicate = units.some(unit => unit.name.toLowerCase() === unitName.trim().toLowerCase() && unit._id !== currentUnit?._id);
    if (isDuplicate) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <header className="flex flex-col sm:flex-row justify-between sm:items-center mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">Manage the product currency units for your inventory. (Based Currency = SGD)</p>
          </div>
          <button
          ...
          <div className="divide-y divide-gray-200">
            {units.length > 0 ? (
              units.map((unit) => (
                ...
                >
                  <span className="font-medium text-lg text-gray-700">{unit.name}</span>
                  <span className="text-gray-500">Rate: {rates[unit.name]?rates[unit.name]:"No rate to display"}</span>
              ...
              <div key={0} className="p-8 text-center text-gray-500">
                <p className="font-medium">No size units found.</p>
                <p className="text-sm mt-1">Click "Add New Unit" to get started.</p>
              </div>
            )}
          </div>
        </main>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md transform animate-scaleIn">
            <form onSubmit={handleSubmit}>
              <div className="p-6">
                ...
                <p className="text-gray-500 mb-6">Enter a new currency unit. For example: "SGD" or "MYR".</p>
                
                <div>
                  <label htmlFor="unitName" className="text-sm font-semibold text-gray-700 mb-2 block">
                    Unit Name
                  </label>
                  <input
                    id="unitName"
              ...
              <div className="bg-gray-50 px-6 py-4 flex justify-end gap-3 rounded-b-xl">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-indigo-600 cursor-pointer text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 transition-colors"
                >
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 animate-fadeIn">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-center transform animate-scaleIn">
                <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-red-100 mb-4">
                   <BsFillTrash3Fill className="h-6 w-6 text-red-600" />
                </div>
                <h3 className="text-lg font-medium text-gray-900">Delete Unit?</h3>
                <div className="mt-2 text-sm text-gray-500">
                    <p>Are you sure you want to delete the unit <span className="font-bold">"{unitToDelete.name}"</span>? This action cannot be undone.</p>
                </div>
                <div className="mt-6 flex justify-center gap-4">
                    <button onClick={cancelDelete} type="button" className="px-4 py-2 bg-white text-gray-700 font-semibold border cursor-pointer border-gray-300 rounded-lg hover:bg-gray-100 transition-colors w-24">
                        Cancel
                    </button>
                    <button onClick={confirmDelete} type="button" className="px-4 py-2 bg-red-600 text-white font-semibold cursor-pointer rounded-lg shadow-md hover:bg-red-700 transition-colors w-24">
                        Delete
                    </button>
                </div>
            </div>
